...
动效也应进入 preference system，而不是只隐含在 OS 媒体查询里。
...
1. `system` 跟随 OS 的 `prefers-reduced-motion`。
2. `reduced` 是用户显式要求，应稳定压低结构位移和持续性动画。
3. `full` 只能在 OS 未要求 reduced 的情况下生效；产品不应用 `full` 去反向覆盖无障碍硬约束。

推荐理解为：

- raw preference: `motionMode`
- resolved motion: `full | reduced`
...
4. 把 `motionMode` 的 raw value 与 resolved motion 的实现契约补到 token / utility 层。
