...
主题应作为正式的长期 preference，而不是临时按钮状态。
...
1. `system` 跟随 OS 或宿主环境主题。
2. `light / dark` 是用户显式偏好，应覆盖产品默认值。
3. 需要区分“原始偏好值”和“最终解析主题”。

推荐理解为：

- raw preference: `themeMode`
- resolved theme: `light | dark`

### 设计约束

1. theme preference 应是全局性的，不建议做页面级或面板级主题分叉。
2. dark theme 不是系统的第二套品牌，应和 light 保持同一产品性格。
3. 当主题变化时，信息层级、ownership 关系和阅读舒适度必须一起成立，而不是只切背景色。

## 三、Dark Theme 需要保住的关系
...
4. 把 `themeMode` 的 raw value 与 resolved theme 的 token 映射补成稳定前端契约。
